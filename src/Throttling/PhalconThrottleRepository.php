...
class PhalconThrottleRepository implements ThrottleRepositoryInterface
{
    use RepositoryTrait;
    ...
    /**
     * Cached global throttles Resultset within the interval.
     *
    ...
    /**
     * The cached IP address throttle Resultsets within the interval.
     *
     * @var array
     */
    ...
    /**
     * The cached user throttle Resultsets within the interval.
     *
     * @var \Phalcon\Mvc\Model\Resultset\Simple
     */
    ...
    public function __construct(
        $model = 'Cartalyst\Sentinel\Throttling\PhalconThrottle',
        $globalInterval = null,
    ...
    public function log($ipAddress = null, UserInterface $user = null)
    {
        ...
        if ($ipAddress !== null) {
            ...
            $ipAddressThrottle->type = (string) 'ip';
            $ipAddressThrottle->ip = (string) $ipAddress;
            $ipAddressThrottle->save();
        }

        if ($user !== null) {
            $userThrottle = $this->createModel();
            $userThrottle->type = (string) 'user';
            $userThrottle->user_id = (int) $user->getUserId();
            $userThrottle->save();
        }
    }
    ...
    protected function delay($type, $argument = null)
    {
        ...
        if (is_array($this->$thresholds)) {
            ...
            foreach (array_reverse($this->$thresholds, true) as $attempts => $delay) {
                ...
                if ($this->createCarbon($last->created_at)->diffInSeconds() < $delay) {
    ...
    /**
     * Returns the global throttles Resultset.
     *
    ...
    /**
     * Loads and returns the global throttles Resultset.
     *
     * @return \Phalcon\Mvc\Model\Resultset\Simple
     */
    ...
    /**
     * Returns the IP address throttles Resultset.
     *
     * @param  string  $ipAddress
     * @return \Phalcon\Mvc\Model\Resultset\Simple
     */
    ...
    /**
     * Loads and returns the IP address throttles Resultset.
     *
     * @param  string  $ipAddress
     * @return \Phalcon\Mvc\Model\Resultset\Simple
     */
    ...
    /**
     * Returns the user throttles Resultset.
     *
     * @param  \Cartalyst\Sentinel\Users\UserInterface  $user
     * @return \Phalcon\Mvc\Model\Resultset\Simple
     */
    ...
    /**
     * Loads and returns the user throttles Resultset.
     *
     * @param  \Cartalyst\Sentinel\Users\UserInterface  $user
     * @return \Phalcon\Mvc\Model\Resultset\Simple
     */
     ...
     *
     * @param  \Cartalyst\Sentinel\Throttling\PhalconThrottle  $throttle
     * @param  int  $interval
     * @return int
     */
    protected function secondsToFree(PhalconThrottle $throttle, $interval)
    {
        return $this->createCarbon($throttle->created_at)->addSeconds($interval)->diffInSeconds();
    }

    /**
     * Returns instance of \Carbon\Carbon from a date
     *
     * @param  string  $dateTime
     * @return \Carbon\Carbon
     */
    protected function createCarbon($dateTime)
    {
