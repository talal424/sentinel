...
class PhalconUserRepository implements UserRepositoryInterface
{
    ...
    /**
     * The Phalcon user model name.
     *
     * @var string
     */
    protected $model = 'Cartalyst\Sentinel\Users\PhalconUser';

    /**
     * Create a new Phalcon user repository.
     *
    ...
    public function findByCredentials(array $credentials)
    {
        ...
        if (is_array($logins)) {
            ...
            foreach ($loginNames as $name) {
                $query->andWhere("$name = :$name:", [$name => $logins]);
            }
        }
    ...
    {
        $PersistencesModel = $this
        ->createModel()
        ->getPersistencesModel();
        
        $persistences = $PersistencesModel::findFirstByCode($code);

        return $persistences ? $persistences->user : null;
    }
    ...
    {
        $user->last_login = (string) Carbon::now();
